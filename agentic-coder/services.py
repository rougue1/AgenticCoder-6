...
client, context tooling, sandbox, registry) so stages take a single argument
instead of a dozen. It is populated in two phases by the orchestrator: the LLM
client and bus exist immediately, while the workspace-dependent services are
built once the project directory is resolved (the slug may come from intake).
"""
...
from context.compressor import Compressor
...
class Progress:
    ...
    The durable task breakdown lives on disk in ``tasks.json``; this captures the
    sub-stage detail that only exists while the subtask loop is running (which of
    plan/implement/test/fix it is on, and when the current subtask started) so the
    UI can show an accurate phase badge and a per-subtask timer.
    """

    activity: str = ""              # plan | implement | test | fix | escalate | ""
    subtask_id: str = ""
    subtask_title: str = ""
    subtask_intent: str = ""
...
class Services:
    ...
    def attach_workspace(self, workspace: Workspace) -> None:
        ...
        self.compressor = Compressor(self.config, self.bus)
        self.builder = ContextBuilder(workspace, self.loader, self.manifest, self.compressor, self.config)
        self.sandbox = Sandbox(workspace, self.config.limits, self.bus)
    ...
    def check_cancel(self) -> None:
        ...
        Called at every atomic boundary in the pipeline (between stages, subtasks,
        and implementer tool calls). Because it is *not* called mid-LLM-stream, a
        pause requested during a stream takes effect once that stream completes —
        exactly the "finish the current atomic operation, then hold" contract.
        """
        self.wait_if_paused()
